...
const SingleAuctionDetail = ({ noPadding }) => {
  ...
  {singleAuction?.status === "over" || auctionWinnerDetailData ? (
        ...
        {logInUser ? (
          user?.paymentVerified ? (
            <button
              type="submit"
              disabled={
                singleAuction?.seller?._id === logInUser?._id || !auctionStarted
              }
    ...
    <button  onClick={handleAddToCart} className="bg-color-primary py-3 px-1 text-sm rounded-md cursor-pointer text-white">
    Add to Cart
  </button>
