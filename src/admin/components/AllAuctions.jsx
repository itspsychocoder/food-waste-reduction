...
const AllAuctions = () => {
  ...
  const handleFilterChange2 = (e) => {
    const value = e.target.value || undefined;
    setFilter1(filterField2, value); // Use setFilter to apply the filter
    setFilterInput2(value); // Update filterInput2 state
  };
  ...
  const columnnswithoutstarttimendendtime = useMemo(
    () => [
      {
        Header: "Picture",
        accessor: "picture",
        Cell: ({ value }) => (
          <img src={value} alt="Profile" style={{ width: "50px", height: "50px" }} />
        ),
      },
      {
        Header: "Name",
        ...
        Cell: ({ row: { original } }) => (
          <div className="flex gap-2">
            ...
            {original.status === "upcoming" && (
              <Link to={`/admin/auctions/edit/${original.actions}`}>
                <FaRegEdit
                  size={38} className="inline mt-[-3px] text-theme-color hover:text-white hover:bg-theme-color rounded-lg border-2 border-theme-color px-2 py-[5px] transition-all"
                />
              </Link>
            )}
          </div>
  ...
  const data1 = useMemo(
    () =>
      Array.isArray(auction)
        ...
        ? auction.filter((auc) => auc?.startTime === null && auc?.endTime === null)
        .map((auc) => {
  ...
  const {
    ...
    {
      columns: columnnswithoutstarttimendendtime,
      data: data1,
      initialState: { pageSize: 8 },
    },
    useFilters,
    ...
    <>
      <div className="px-7 py-4 w-full bg-theme-bg text-slate-300 rounded-2xl ">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          >
            <option value="select">Select a Field</option>
...
/>
      ...
      <div className="overflow-auto px-4 rounded-2xl border border-border-info-color mt-4 max-h-[500px]">
        ...
        >
          <thead className="sticky top-0 bg-theme-bg table-header-group">
            {headerGroups1.map((headerGroup, headerGroupIndex) => (
              <tr
                className="table-row"
                {...headerGroup.getHeaderGroupProps()}
                key={headerGroupIndex}
          ...
          <tbody className="table-row-group" {...getTableBodyProps1()}>
            {page1.map((row) => {
              prepareRow1(row);
              const rowProps = row.getRowProps();
              return (
      ...
      <div className="flex flex-col md:flex-row justify-center gap-4 my-4 items-center md:button:px-4 md:button:py-2 button:px-2 button:py-1 button:rounded-lg  ">
          <div className="flex justify-center items-center gap-4">
            <button
              className={`bg-[#00A3FF] hover:bg-color-danger text-white transition-all ${
                pageIndex1 === 0 ? "bg-gray-500 hover:bg-gray-500" : ""
              }`}
              onClick={() => gotoPage1(0)}
              disabled={pageIndex1 === 0 ? true : false}
            >
              <MdSkipPrevious size={18} />{" "}
            </button>
            <button
              onClick={() => previousPage1()}
              disabled={!canPreviousPage1}
              className={`bg-[#00A3FF] hover:bg-color-danger text-white transition-all ${
                !canPreviousPage1 ? "bg-gray-500 hover:bg-gray-500" : ""
              }`}
            >
              <FaCaretLeft size={18} />
            </button>
            ...
            <button
              ...
              className={`bg-[#00A3FF] hover:bg-color-danger text-white transition-all ${
                !canNextPage1 ? "bg-gray-500 hover:bg-gray-500" : ""
              }`}
            >
              <FaCaretRight size={18} />
            </button>
            <button
              className={`bg-[#00A3FF] hover:bg-color-danger text-white transition-all ${
                pageIndex === pageCount1 - 1
                  ? "bg-gray-500 hover:bg-gray-500"
                  : ""
              }`}
              onClick={() => gotoPage1(pageCount1 - 1)}
            >
              <MdSkipNext size={18} />{" "}
            </button>
          </div>
